...
from resource_manager import res_mgr,music_manager,ending_manager
...
class Level10(BaseLevel):
    ...
    def __init__(self, screen, res_mgr):
        ...
        self.effectwakeup_2 = res_mgr.get_sound("effect_wakeup_2")
        ...
        self.current_game_state = GameState.LEVEL10
        print(f"ending_manager exists: {hasattr(self, 'ending_manager')}")
    ...
    def check_password(self):
        ...
        elif ending_manager.is_True_end_get and self.input_text == "123456":  # 后门密码
            self.show_welcome = True
            self.welcome_start_time = pygame.time.get_ticks()
            self.input_text = ''
            self.effectwakeup_2.play()
            ending_manager.is_True_end_start = True
            print("true end start")
        else:
    ...
    def update(self):
        ...
        if self.transition_over:
            self.show_welcome = False
            if ending_manager.is_True_end_start and ending_manager.is_True_end_get  :
                config.current_state = GameState.LEVEL12#真结局线-真结局
            else:
                config.current_state = GameState.LEVEL11#假结局线
    ...
    def draw(self):
        ...
        if config.current_state == GameState.LEVEL11:  # 防止过渡完成后原场景会闪现一下
            self.screen.fill((0, 0, 0))
